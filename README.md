...
## Testing
- Added a span tag as a counter for cart items.
- Defined an array and assigned no values to it as it will later be called using jquery.
- Added necessary classes to elements with descriptive namings
- Used jquery to add click event to buttons using classes so that they are easily identified when called.
- Looks okay will need to work on removing item from the cart. Might require assistance again.
